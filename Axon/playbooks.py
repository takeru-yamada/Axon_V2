...
class PlaybookEngine:
    ...
    def _analyze_with_ai(state, mode: str):
        """Ask Gemini, then Mistral, to analyze this case's isolated evidence."""
        ...
        system = (
            ...
            "complex constraints in everyday language. "
            "Do not invent facts."
        )
        errors = []
        if gemini_keys:
    ...
    def inspect_case(case_id: str):
        """Run a generic edge-constraint check before the agent pipeline starts."""
        ...
        has_constraint_language = any(term in prompt for term in (
            "edge constraint", "edge case", "constraint", "must be", "exactly", "zero ",
            "0 ", "immediately", "always", "never", "guarantee", "without ", "cannot",
            "budget", "under budget", "within budget", "cost limit", "price limit",
        ))
        has_impossible_number = bool(re.search(
            r"(?:\b0\s*(minutes?|mins?|hours?|seconds?)\b|\b-\d+\b|\b\d{3,}%\b)", prompt
        ))
        has_conflicting_absolutes = bool(re.search(
            r"\b(always|must)\b.{0,80}\b(never|cannot|without)\b|\b(never|cannot)\b.{0,80}\b(always|must)\b",
            prompt,
        ))
        if has_impossible_number or has_conflicting_absolutes:
            questions.append(
                "Please clarify this constraint: should I use a realistic value instead of the absolute or contradictory limit?"
            )
        elif has_constraint_language:
            questions.append(
                "Please confirm the edge constraint and provide the acceptable range or exception rule."
            )
