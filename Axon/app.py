...
def get_sandbox_state(case_id):
    ...
    sandbox_states = st.session_state.setdefault("sandbox_states", {})
    state = sandbox_states.setdefault(case_id, {})
...
def save_active_case():
    ...
    if case_id:
        st.session_state.cases.setdefault(case_id, {})
        st.session_state.cases[case_id].update({
...
def activate_case(case_id: str):
    save_active_case()
    case = st.session_state.cases.setdefault(case_id, {})
    st.session_state.exception_case_id = case_id
    st.session_state.current_task = case.get("task", "")
    st.session_state.pipeline_done = case.get("pipeline_done", False)
    st.session_state.pipeline_data = case.get("pipeline_data", {})
    state = get_sandbox_state(case_id)
    if state["required_questions"]:
        st.session_state.active_user_modal_case = case_id
    else:
...
with st.sidebar:
    ...
    if st.session_state.cases:
        ...
        selected_case = st.selectbox(
            "Active case",
            case_ids,
            index=case_ids.index(st.session_state.exception_case_id)
            if st.session_state.exception_case_id in case_ids else 0,
            format_func=lambda case_id: f"Case {case_id[:12]}...",
        )
        if selected_case != st.session_state.exception_case_id:
            activate_case(selected_case)
            st.rerun()
    if st.button("+ New case", key="new_case", use_container_width=True):
        case_id = generate_request_id()
        st.session_state.cases[case_id] = {
            "task": "", "pipeline_done": False, "pipeline_data": {},
        }
        get_sandbox_state(case_id)
        st.session_state.exception_case_id = case_id
...
if (
    st.session_state.get("active_user_modal_case")
    and st.session_state.get("active_user_modal_case") == st.session_state.get("exception_case_id")
):
...
if page == "home":
    ...
    if send and user_input.strip():
        if not st.session_state.api_keys:
            ...
            case_id = generate_request_id()
            st.session_state.cases[case_id] = {
                "task": task, "pipeline_done": False, "pipeline_data": {},
            }
            activate_case(case_id)
            get_sandbox_state(case_id)["case_data"] = {
                "user_prompt": task,
                "original_prompt": task,
            }
            PlaybookEngine.inspect_case(case_id)
