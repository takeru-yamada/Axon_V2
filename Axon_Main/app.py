...
import ast, html, json, math, re, time, random
from datetime import datetime
from pathlib import Path
from typing import Any

from app_config import DEFAULT_API_KEYS, DEFAULT_MISTRAL_KEYS, MISTRAL_MODEL
from app_utils import classify_error, clean_display_text, safe_json
from ai_client import gemini_json, gemini_text
from pages import render_builder_page, render_help_page
from pipeline_core import (
    ...
    stage_critic,
    stage_dwsa,
    stage_quantum_routing,
    stage_seo,
    stage_synthesis,
    stage_verifier,
)
from playbooks import PlaybookEngine, route_and_execute
from popup import show_missing_data_popup
from reporting import ReporterFinding, ReporterAgent, generate_report_request
from uuid_gen import generate_request_id, get_sandbox_state
...
with st.sidebar:
    st.markdown('<div class="sidebar-logo">🌸 Axon - Agent</div>', unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown('<div style="font-size:0.62rem;color:#3b2a5a;font-family:\'JetBrains Mono\',monospace;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.4rem;">Divided by geography, united by tech</div>', unsafe_allow_html=True)
    ...
    st.image("sideimg.png", use_container_width=True)
...
if page == "builder":
    render_builder_page(BUILDER, render_torii)
    st.stop()
if page == "help":
    render_help_page(gemini_text, render_torii)
    st.stop()
